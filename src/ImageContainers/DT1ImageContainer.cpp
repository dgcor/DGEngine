...
DT1ImageContainer::Header::Header(LittleEndianStreamReader& fileStream)
{
...
int DT1ImageContainer::Tile::SUBTILE_INDEX[SUBTILE_SIZE][SUBTILE_SIZE] = {};
int DT1ImageContainer::Tile::SUBTILE_OFFSET[NUM_SUBTILES][2] = {};
DT1ImageContainer::Tile::Tile(LittleEndianStreamReader& fileStream)
{
...
sf::Image2 DT1ImageContainer::Tile::decode(const PaletteArray* palette) const
{
    int absWidth = width;
...
void DT1ImageContainer::Tile::drawIsometricBlock(sf::Image2& img, const PaletteArray* palette, int x0, int y0, const uint8_t* data, int length) const
{
...
void DT1ImageContainer::Tile::drawRLEBlock(sf::Image2& img, const PaletteArray* palette, int x0, int y0, const uint8_t* data, int length) const
{
    int i = 0, x = 0, y = 0;
    while (length > 0)
    {
...
int DT1ImageContainer::Tile::createIndex(int orientation, int mainIndex, int subIndex)
{
...
int DT1ImageContainer::Tile::getMainIndex(int index) { return (index >> MAIN_INDEX_OFFSET) & MAIN_INDEX_BITS; }

int DT1ImageContainer::Tile::getSubIndex(int index) { return (index >> SUB_INDEX_OFFSET) & SUB_INDEX_BITS; }

int DT1ImageContainer::Tile::getOrientation(int index) { return (index >> ORIENTATION_OFFSET) & ORIENTATION_BITS; }

DT1ImageContainer::Tile::Block::Block(LittleEndianStreamReader& fileStream)
{
    uint8_t zeros1[2];
    uint8_t zeros2[2];
    fileStream.read(x);
...
int DT1ImageContainer::Tile::Orientation::getDirection() const
{
    switch (value)
    {
        case LEFT_WALL:
...
bool DT1ImageContainer::Tile::Orientation::isFloor() const
{
    switch (value)
    {
        case FLOOR:
...
bool DT1ImageContainer::Tile::Orientation::isRoof() const
{
    switch (value)
    {
        case ROOF:
...
bool DT1ImageContainer::Tile::Orientation::isWall() const
{
    switch (value)
    {
        case LEFT_WALL:
...
bool DT1ImageContainer::Tile::Orientation::isSpecial() const
{
    switch (value)
    {
        case SPECIAL_10:
...
DT1ImageContainer::DT1ImageContainer(const std::string_view fileName)
{
	...
	header = DT1ImageContainer::Header(fileStream);

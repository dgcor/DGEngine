...
uint16_t Cl2ImageContainer::computeWidthFromHeader(const gsl::span<const uint8_t> frameData)
{
...
sf::Image2 Cl2ImageContainer::decode(const gsl::span<const uint8_t> frameData,
	...
	if (type == CelType::V2MultipleGroups)
	{
		...
		for (uint32_t i = 0; i * 4 < firstDword; i++)
		{
			...
			groupFrameIndexes.push_back(
				...
				(uint16_t)(frameOffsets.size() + groupFrameCount - 1)));
...
sf::Image2 Cl2ImageContainer::get(size_t index, const PaletteArray* palette) const
{

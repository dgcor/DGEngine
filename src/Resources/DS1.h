...
// DS1 decoding code based on Diablo 2 Engine Riiablo by Collin Smith
...
namespace DS1
{
	...
	public:
		...
		Path(LittleEndianStreamReader& fileStream, std::unordered_map<int, Object>& objects,
			int32_t version)
		{
	...
	public:
		...
		std::unordered_map<int, Cell> walls;

		int32_t floorLine, floorLen;
		std::unordered_map<int, Cell> floors;

		int32_t shadowLine, shadowLen;
		std::unordered_map<int, Cell> shadows;

		int32_t tagLine, tagLen;
		std::unordered_map<int, int32_t> tags;
		...
		std::unordered_map<int, std::pair<int, int>> specials;

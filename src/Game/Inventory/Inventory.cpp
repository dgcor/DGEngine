...
bool Inventory::set(size_t idx, std::shared_ptr<Item>& item, std::shared_ptr<Item>& oldItem)
{
	if (idx < items.size())
	{
		if (item != nullptr &&
...
bool Inventory::set(const PairUInt8& position, std::shared_ptr<Item>& item, std::shared_ptr<Item>& oldItem)
{
...
bool Inventory::setAndDontEnforceItemSize(size_t idx, std::shared_ptr<Item>& item, std::shared_ptr<Item>& oldItem)
{
...
bool Inventory::setAndEnforceItemSize(const PairUInt8& position, std::shared_ptr<Item>& item, std::shared_ptr<Item>& oldItem)
{
	if (position.x >= size.x || position.y >= size.y ||
		size.x == 0 || size.y == 0)
	{
...
bool Inventory::isSlotEmpty(int x, int y, const PairUInt8& itemSize, size_t& itemIdx) const
{
...
bool Inventory::getFreeSlot(const Item& item, size_t& itemIdx, InventoryPosition invPos) const
{
...
uint32_t Inventory::findFreeQuantity(uint16_t classIdHash16, size_t& idx, Item*& itemFound) const
{
	auto size = items.size();
	if (idx < size)
	{
...
uint32_t Inventory::findBiggestFreeQuantity(uint16_t classIdHash16, Item*& itemFound, uint32_t quantityNeded) const
{
...
uint32_t Inventory::findSmallestFreeQuantity(uint16_t classIdHash16, Item*& itemFound, uint32_t quantityNeded) const
{
...
unsigned Inventory::countFreeSlotsNoChecks() const
{
	unsigned count = 0;
	for (const auto& item : items)
	{
...
unsigned Inventory::countFreeSlots(uint16_t typeHash16) const
{
	unsigned count = 0;
	if (isTypeAllowed(typeHash16) == true)
	{
		count = countFreeSlotsNoChecks();
	}
	return count;
}

LevelObjValue Inventory::addQuantity(const ItemClass& itemClass, LevelObjValue amount, InventoryPosition invPos, Queryable* itemOwner)
{
	if (amount == 0 ||
...
uint32_t Inventory::getMaxCapacity(const ItemClass& itemClass) const
{
	...
	auto freeSlots = countFreeSlotsNoChecks();
	if (freeSlots > 0)
	{
...
bool Inventory::getProperty(const std::string_view prop, Variable& var) const
{
	...
	case str2int16("itemCount"):
		...
		break;
	case str2int16("size"):
		var = UIObjectUtils::getTuple2iProp(size, props.second);
		break;
	break;
	default:
		return false;

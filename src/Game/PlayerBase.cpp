...
void PlayerBase::updateWalkPathStep(PairFloat& newMapPos)
{
...
void PlayerBase::updateWalkPath(Game& game, LevelMap& map)
{
...
bool PlayerBase::getTexture(uint32_t textureNumber, TextureInfo& ti) const
{
...
void PlayerBase::setProperty(const std::string_view prop, const Variable& val)
{
	if (prop.empty() == true)
	{
		return;
...
const Queryable* PlayerBase::getQueryable(const std::string_view prop) const
{
	if (prop.empty() == true)
	{
		return this;
...
bool PlayerBase::hasInt(const std::string_view prop) const noexcept
{
	return hasIntByHash(str2int16(prop));
}

bool PlayerBase::getNumberProp(const std::string_view prop, Number32& value) const
{
	if (prop.empty() == true)
	{
		return false;
...
bool PlayerBase::getNumberByHash(uint16_t propHash,
...
bool PlayerBase::getCustomIntByHash(uint16_t propHash, Number32& value) const noexcept
{
	return customProperties.getValue(propHash, value);
}

bool PlayerBase::getCustomInt(const std::string_view prop, Number32& value) const noexcept
{
...
bool PlayerBase::getInt(const std::string_view prop, LevelObjValue& value) const noexcept
{
...
bool PlayerBase::setIntByHash(uint16_t propHash, LevelObjValue value, const Level* level) noexcept
{
	switch (propHash)
	{
	case str2int16("strength"):
...
bool PlayerBase::setInt(const std::string_view prop, LevelObjValue value, const Level* level) noexcept
{
...
bool PlayerBase::setNumberByHash(uint16_t propHash, LevelObjValue value, const Level* level) noexcept
{
...
bool PlayerBase::setNumber(const std::string_view prop, LevelObjValue value, const Level* level) noexcept
{
...
void PlayerBase::updateNameAndDescriptions() const
{
...
LevelObjValue PlayerBase::addItemQuantity(const ItemClass& itemClass,
...
void PlayerBase::updateItemQuantityCache(uint16_t classIdHash16, LevelObjValue amount) noexcept
{
...
void PlayerBase::updateItemQuantityCache(uint16_t classIdHash16) noexcept
{
	auto quantity = itemQuantityCache.getValue(classIdHash16);
	if (quantity != nullptr)
	{
...
bool PlayerBase::getFreeItemSlot(const Item& item, size_t& invIdx,
...
void PlayerBase::addSpell(const std::string_view key, Spell* obj, LevelObjValue spellLevel)
{
...
bool PlayerBase::setItemInFreeSlot(size_t invIdx, std::shared_ptr<Item>& item,
...
void PlayerBase::updateBodyItemValues()
{
	strengthItems = 0;
	magicItems = 0;
	dexterityItems = 0;
...
void PlayerBase::updateProperties()
{
...
bool PlayerBase::hasMaxStats() const noexcept
{

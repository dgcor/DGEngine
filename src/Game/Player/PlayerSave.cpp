...
void PlayerSave::serialize(const PlayerBase& player, const Level& level, void* serializeObj, const UnorderedStringMap<Variable>& props)
{
	...
	if (level.LevelObjects().CurrentPlayer().get() == &player)
	{
	...
	writer.StartObject();
	{
		...
		for (const auto& prop : player.properties.custom)
		{
	...
	if (player.inventories.empty() == false)
	{
		...
		for (size_t i = 0; i < player.inventories.size(); i++)
		{
			...
			if (i == player.inventories.getBodyInventoryIdx())
			{
			...
			if (inv.empty() == false)
			{
				...
				for (auto it = inv.begin(); it != inv.end(); ++it)
				{
					...
					it->serialize(level, serializeObj, itemProps);
				}
				writer.EndArray();
			}

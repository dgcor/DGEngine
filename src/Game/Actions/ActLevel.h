...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			auto drawable = level->Drawables().get(idDrawable);
			if (drawable != nullptr)
			{
				drawable->anchorTo = level->LevelObjects().getByQueryId(idAnchor);
...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			level->LevelObjects().clear<LevelObject>(level->Map(), excludeIds);
		}
		return true;
	}
};
...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			level->LevelObjects().clear<Item>(level->Map(), excludeIds);
		}
		return true;
	}
};
...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			level->LevelObjects().clear<Player>(level->Map(), excludeIds);
		}
		return true;
	}
};
...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			auto player = level->LevelObjects().get<Player>(idPlayer);
			if (player != nullptr)
			{
...
private:
	std::string id;
	std::string file;
	UnorderedStringMap<Variable> props;

public:
	ActLevelSave(const std::string_view id_, const std::string_view file_,
		UnorderedStringMap<Variable>&& props_) : id(id_), file(file_), props(std::move(props_)) {}
	...
	bool execute(Game& game) noexcept override
	{
		...
		if (level != nullptr)
		{
			level->save(GameUtils::replaceStringWithVarOrProp(file, game), props);
		}
		return true;
	}
};
...
public:
	...
	bool execute(Game& game) override
	{
		...
		if (level != nullptr)
		{
			level->Drawables().update(*level, game);
		}
		return true;
	}
};

...
TexturePackVariant MonsterClass::getTexturePack(size_t idx) const
{
...
void MonsterClass::getTextureAnimationRange(size_t textureIdx,
...
AnimationSpeed MonsterClass::getSpeed(PlayerAnimation animation) const noexcept
{
...
void MonsterClass::setSpeed(PlayerAnimation animation, const AnimationSpeed& speed)
{
	for (auto& elem : animationSpeeds)
	{
		if (elem.first == animation)
...
const sf::SoundBuffer* MonsterClass::getSound(size_t idx) const noexcept
{
	if (idx < sounds.size())
	{
		return sounds[idx];
...
const sf::SoundBuffer* MonsterClass::getSound(size_t idx, size_t size) const
{
	if (size <= 1)
	{
		return getSound(idx);
...
void MonsterClass::setSound(size_t idx, const sf::SoundBuffer& snd) noexcept
{
	if (idx < sounds.size())
	{
		sounds[idx] = &snd;
...
void MonsterClass::setFormula(uint16_t nameHash, const std::string_view formula)
{

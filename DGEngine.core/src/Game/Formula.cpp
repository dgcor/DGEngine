	...
	for (; it.isValid() == true; it.next())
	{
			...
			std::holds_alternative<std::string_view>(elem) == true)
		{
			...
			if (prop.empty() == false)
			{
				...
				{
					if (queryA != nullptr &&
						queryA->getNumber(prop, queryVal) == true)
					{
				...
				{
					if (queryB != nullptr &&
						queryB->getNumber({ prop.data() + 1 , prop.size() - 1 }, queryVal) == true)
					{
						val2 = queryVal.getDouble();
					}
				}
		...
		case FormulaOp::RandFloat:
		{
			if (randomNum == 0)
			{
				if (val2 > 0.0)
				{
					...
					{
						val2 = (double)Random::get((uint32_t)val2);
					}
					else
					{
						val2 = Random::getf(val2);
					}
				}
				else
				{
		...
		case FormulaOp::RandNormalDist:
		{
			if (randomNum == 0)
			{
				if (val2 > 0.0)
				{
					val2 = std::round(RandomNormal::getRange(val2));
				}
				else
				{
					val2 = 0.0;

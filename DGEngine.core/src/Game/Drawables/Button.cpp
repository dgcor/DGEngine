	...
	if (enabled == true)
	{
		if (action != nullptr)
		{
			if (focusEnable == true)
			{
				...
				{
					game.Events().tryAddBack(focusAction);
				}
			}
			if (playSound == true)
			{
...
void Button::focus(Game& game) const
{
	...
	game.Events().tryAddFront(focusAction);
}
...
void Button::onMouseButtonPressed(Game& game, bool contains)
{
	...
	case sf::Mouse::Left:
	{
		...
		game.Events().tryAddBack(clickInAction);
		if (clickUp == false)
		{
...
void Button::onMouseButtonReleased(Game& game, bool contains)
{
	...
	case sf::Mouse::Left:
	{
		...
		beingDragged = false;
		game.Events().tryAddBack(clickOutAction);
	}
	break;
	case sf::Mouse::Right:
	{
...
void Button::onMouseMoved(Game& game)
{
	...
	{
		game.Events().tryAddBack(clickDragAction);
	}
}

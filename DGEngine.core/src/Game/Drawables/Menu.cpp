...
void Menu::addItem(const std::shared_ptr<StringButton>& item, bool recalculatePositions_)
{
	items.push_back(item);
	if (recalculatePositions_ == true ||
...
void Menu::calculatePositions()
{
	...
	{
		auto offset = pos.y - currPos.y + lastSize;
		if (vertAlign == VerticalAlign::Center)
		{
			offset = std::round(offset / 2.f);
		}
		for (auto& item : items)
		{
...
void Menu::update(Game& game)
{
	...
	for (size_t i = 0; i < items.size(); i++)
	{
		...
		if (focusEnable == true)
		{
			if (button == focus)
			{
				if (recalculateVisibleItems == false)
				{
					recalculateVisibleItems = (currentIdx != i);
				}
				currentIdx = i;
			}
		}
	...
	if ((recalculatePositions == true ||
		recalculateVisibleItems == true) &&
		visibleItems > 0)
	{
		recalculatePositions = true;
		calculatePositions();
		recalculateVisibleItems = false;
	}
	...
	if (game.wasMouseScrolled() == true)
	{
		...
		if (scrollRect.contains(scroll.x, scroll.y))
		{
			...
			if (scroll.delta < 0.f)
			{
				game.Events().tryAddBack(scrollDownAction);
			}
			else
			{
				game.Events().tryAddBack(scrollUpAction);
			}
		}
	}
}

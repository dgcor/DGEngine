...
public:
	...
	{
		Variable value2;
		if (game.Variables().getVariableNoToken(key, value2) == true)
		{
				...
				std::holds_alternative<int64_t>(value) == true)
			{
				...
				game.Variables().setVariable(key, value2);
			}
...
public:
	...
	bool execute(Game& game) override
	{
		if (key.empty() == false)
		{
			...
			{
				game.Variables().setVariable(key, val);
			}
			else
			{
...
public:
	...
	bool execute(Game& game) override
	{
		...
		{
			Variable var;
			if (game.Variables().getVariableNoToken(key, var) == false)
			{
				if (resolveValue == false)
				{
					game.Variables().setVariable(key, val);
	...
	bool execute(Game& game) override
	{
		for (const auto& keyVar : variables)
		{
			if (keyVar.first.empty() == false)
			{
				if (resolveValue == false)
				{
					game.Variables().setVariable(keyVar.first, keyVar.second);
				}
				else
				{
					auto var = game.getVarOrProp(keyVar.second);
					game.Variables().setVariable(keyVar.first, var);
				}
			}
		}
		return true;

...
void ResourceManager::setInputAction(const sf::Event& evt, const std::shared_ptr<Action>& obj)
{
	if (evt.type == sf::Event::KeyPressed &&
		(evt.key.code == sf::Keyboard::KeyCount ||
			evt.key.scancode == sf::Keyboard::Scancode::ScancodeCount))
	{
...
std::shared_ptr<Action> ResourceManager::getInputAction(const sf::Event& key) const
{
	for (const auto& res : reverse(resources))
	{
		...
		auto evt = key;
		auto keyCode = sf::Keyboard::Unknown;
		...
		const auto elem = res.inputActions.find(evt);
...
void ResourceManager::addFocused(const std::shared_ptr<Button>& obj, const std::string_view resourceId)
{
	if (resourceId.empty() == true)
	{
...
std::shared_ptr<Button> ResourceManager::getFocused() const
{
	for (const auto& res : reverse(resources))
	{
		...
		if (res.focusButtons.empty() == false &&
			res.focusIdx < res.focusButtons.size())
		{
			return res.focusButtons[res.focusIdx].lock();
		}
		break;
	}
	return nullptr;

#include "ParseConditionActions.h"
#include "Game/Actions/ActCondition.h"
...
namespace Parser::Actions
{
	...
	std::shared_ptr<Action> parseIfCondition(uint16_t conditionHash16, Game& game, const Value& elem)
	{
	...
	std::shared_ptr<Action> parseInListCondition(Game& game, const Value& elem)
	{
	...
	void parseMultiIfConditionHelper(Game& game, IfCondition& conditions, const Value& elem)
	{
		size_t step = 0;
	...
	std::shared_ptr<Action> parseMultiIfCondition(Game& game, const Value& elem)
	{
		...
		parseMultiIfConditionHelper(game, action->Conditions(), elem["params"sv]);
		return action;
	}

	std::shared_ptr<Action> parseSwitchConditionHelper(
	...
	std::shared_ptr<Action> parseSwitchCondition(Game& game, const Value& elem)
	{
					...
					val.HasMember("value"sv) == true)
				{
					cases.push_back(
						std::make_pair(
							...
							parseSwitchConditionHelper(game, val, "action", cases)
						)
					);
				}
			}

	...
	: texture(tex), palette(palette_)
{
	...
	for (size_t i = 0; i < charRects.size(); i++)
	{
		...
		auto charSizeIdx = (i * stepNBytes) + charStartIdx;
...
sf::Vector2i BitmapFontTexturePack::getTextureSize(uint32_t index) const noexcept
{
	return index < charRects.size() ? charRects[index].getSize() : sf::Vector2i();
}
